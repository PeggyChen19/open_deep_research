            ...
            if line and line.startswith("data:"):
                ...
                try:
                    ...
                    if "research_brief" in json_obj and not got_research_brief:
                        ...
                        yield f"data: {json.dumps({'type': 'research_brief', 'content': json_obj['research_brief'], 'total_time': global_time, 'phase_time': phase_time})}\n\n"
                        phase_start_time = time.time()

                    if "compressed_research" in json_obj:
                        global_time = int(time.time() - global_start_time)
                        phase_time = int(time.time() - phase_start_time)
                        yield f"data: {json.dumps({'type': 'chain_of_thought', 'content': json_obj['compressed_research'], 'total_time': global_time, 'phase_time': phase_time})}\n\n"
                        phase_start_time = time.time()

                    if "final_report" in json_obj:
                        global_time = int(time.time() - global_start_time)
                        phase_time = int(time.time() - phase_start_time)
                        yield f"data: {json.dumps({'type': 'final_report', 'content': json_obj['final_report'], 'total_time': global_time, 'phase_time': phase_time})}\n\n"

                except json.JSONDecodeError:
                    global_time = int(time.time() - global_start_time)
                    phase_time = int(time.time() - phase_start_time)
                    yield f"data: {json.dumps({'type': 'raw', 'content': data, 'total_time': global_time, 'phase_time': phase_time})}\n\n"
